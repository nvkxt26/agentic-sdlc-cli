...
Entry points: `/resolve-ticket` (one ticket), `/resolve-pr-review` (evaluate and resolve PR review comments), `/mimir` (a question), `/plan-epic` (an epic across repos).
...
- **TOON + caveman FULL (default)** — inter-agent hand-offs use TOON notation with caveman FULL compression by default. Bypass per run with `--no-toon` (plain Markdown) and/or `--no-caveman` (normal prose). See `{{INSTRUCTIONS_DIR}}/toon-communication.instructions.md` and `{{INSTRUCTIONS_DIR}}/caveman.instructions.md`.
...
- **Code only** — the developer and QA stages produce real implementation code and tests, never comment-only stubs. See `{{INSTRUCTIONS_DIR}}/code-style.instructions.md`.
