...
3. **Refresh context.** Run `agentic-workflow run context-sync -- --base <baseRefName> --context-dir {{CONTEXT_DIR}}`. If it reports `full`/`incremental` (or a rebuild reason), refresh via the **context-builder** agent before designing so the baseline reflects the real current tree, not a stale index.
