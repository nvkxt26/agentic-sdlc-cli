...
3. **Route each child ticket to its repo(s)** by matching intent against each repo's published context; when unclear, ask the repo's **Repo Q&A** agent via `repo-bridge -- ask` / `repo-bridge -- answers`. A ticket may span multiple repos — split it into per-repo work items.
