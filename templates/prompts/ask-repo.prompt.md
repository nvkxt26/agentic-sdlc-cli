...
1. **Refresh context if stale.** Run `agentic-workflow run context-sync -- --context-dir {{CONTEXT_DIR}}`.
...
2. **Relationship questions** ("what connects X to Y", "what breaks if I change Z") — check `agentic-workflow run graphify -- status`; if `available: true`, prefer `graphify -- query "<question>"` (or `path`/`explain`) over re-reading files. Skip silently if unavailable — the TOON context is the guaranteed fallback.
